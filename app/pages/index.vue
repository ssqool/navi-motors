<script setup lang="ts">
import { pageHeroImages } from '~/data/gallery'

definePageMeta({ layout: 'default' })

useSeo({
  title: 'Navi Motors — СТО у Києві | Ремонт авто та діагностика',
  description:
    'Navi Motors — СТО у Києві, вул. Віталія Скакуна, 26. Діагностика, ТО, ремонт авто. ☎ 098 535 50 80. Заявка — передзвонимо.',
  ogDescription:
    'Navi Motors — СТО у Києві. Діагностика, ТО та ремонт авто. Заявка на сайті — передзвонимо.',
  path: '/',
})
</script>

<template>
  <div>
    <SeoJsonLd page-type="home" />
    <HeroSection
      title="СТО Navi Motors у Києві"
      subtitle="Діагностика, технічне обслуговування та ремонт авто під ключ. Пояснюємо проблему, погоджуємо вартість до початку робіт і виконуємо ремонт без зайвих питань."
      :image="pageHeroImages.home.src"
      :image-alt="pageHeroImages.home.alt"
      :image-position="pageHeroImages.home.position"
    />
    <ServicesGrid :limit="8" show-all-link />
    <BenefitsSection />
    <PriceTable />
    <SeoLocalSection />
    <GallerySection />
    <ReviewsSection />
    <HomeFaqSection />
    <LeadForm source-page="Головна" />
    <MapSection />
    <SocialSection />
  </div>
</template>
